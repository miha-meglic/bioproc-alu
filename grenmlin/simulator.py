...
def get_steady_single(
    ...
    while True:
        sol = solve_ivp(
            model, [0, t_step], states[-1], dense_output=True, method="LSODA"
        )  # gre za stiff problem, uporaba LSODA
        z = sol.sol(T)
        Y = z.T
...
def simulate_single(
    ...
    sol = solve_ivp(
        model, [0, t_end], S0, dense_output=True, method="LSODA"
    )  # gre za stiff problem, uporaba LSODA
